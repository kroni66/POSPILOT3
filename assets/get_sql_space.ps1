...
try {
    ...
    SELECT 
        ...
        CAST((SUM(CASE WHEN type_desc = 'ROWS' THEN size - FILEPROPERTY(name, 'SpaceUsed') ELSE 0 END) * 8.0 / 1024) AS DECIMAL(10,2)) AS FreeSpaceMB,
        CAST((SUM(CASE WHEN type_desc = 'ROWS' THEN size - FILEPROPERTY(name, 'SpaceUsed') ELSE 0 END) * 100.0 / SUM(CASE WHEN type_desc = 'ROWS' THEN size ELSE 0 END)) AS DECIMAL(5,2)) AS FreeSpacePercent
...
'@
    ...
    $sqlcmdArgs = @(
        "-S", $Server,
        "-U", "POSDOMAIN\eczms909",
        "-P", "Faktnevim123+",
        "-Q", $query,
        "-h", "-1"
    )

    $rawResult = & sqlcmd @sqlcmdArgs
    ...
    if ($null -eq $rawResult -or $rawResult -eq '') {
        ...
        $cleanResult = $rawResult -join '' -replace '[\r\n]', ''
    ...
    @{
        ...
        rawOutput = if ($rawResult) { $rawResult -join "`n" } else { "No output" }
